...
export class DashboardComponent {
  ...
  createLineChart() {
    ...
    new Chart(workoutCtx, {
      type: 'line',
      data: {
        labels: this.workouts.map((data: {date: any;}) =>  this.datePipe.transform(data.date, 'yyyy-MM-dd')),
        datasets: [{
          label: 'Calories Burned',
          data: this.workouts.map((data: {caloriesBurnt: any;}) => data.caloriesBurnt),
          fill: false,
          borderWidth: 2,
    ...
    new Chart(activityCtx, {
      type: 'line',
      data: {
        labels: this.activities.map((data: {date: any;}) =>  this.datePipe.transform(data.date, 'yyyy-MM-dd')),
        datasets: [{
          label: 'Calories Burned',
          data: this.activities.map((data: {caloriesBurnt: any;}) => data.caloriesBurnt),
          fill: false,
          borderWidth: 2,
          backgroundColor: 'rgba(80, 200, 120, 0.6)',
          borderColor: 'rgba(0, 100, 0, 1)'
